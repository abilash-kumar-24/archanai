...
export default function HomePage() {
  return (
    <div className="flex flex-col min-h-full">
      ...
      {/* Hero — the lamp that opens every ceremony */}
      <section className="dusk-section relative overflow-hidden">
        <div className="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 pt-20 pb-24 flex flex-col items-center text-center">
          <Reveal>
            <Flame withBase glow className="h-28 w-auto mx-auto mb-8" />
          </Reveal>
          <Reveal delay={150}>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold italic tracking-tight leading-[1.1] mb-6 text-[#f2ead9]">
              Book a trusted priest{" "}
              <span className="text-flame not-italic">for every ceremony</span>
            </h1>
            <p className="text-lg text-[#f2ead9]/70 leading-relaxed mb-8 max-w-xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center mb-8">
              ...
              <Button
                size="lg"
                variant="outline"
                className="h-12 text-base border-[#f2ead9]/30 bg-transparent text-[#f2ead9] hover:bg-white/10 hover:text-[#f2ead9]"
                asChild
              >
            ...
            <div className="flex flex-wrap justify-center gap-x-5 gap-y-2 text-xs text-[#f2ead9]/60">
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-flame" /> 125+ verified priests</span>
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-flame" /> 6 ceremony types</span>
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-flame" /> 5 cities</span>
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-flame" /> Samagri included</span>
            </div>
          </Reveal>
        </div>
      </section>
              ...
              return (
                <Reveal key={type} delay={i * 60}>
                  <Link
                    ...
                    className="group flex flex-col items-center gap-3 rounded-lg border border-border bg-card p-4 text-center hover:border-primary/50 hover:shadow-md transition-all"
                  >
              ...
              return (
                <Reveal key={city} delay={i * 60}>
                  <Link
                    ...
                    className="group flex flex-col gap-1 rounded-lg border border-border bg-card p-4 hover:border-primary/50 hover:shadow-md transition-all"
                  >
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {HOW_IT_WORKS.map((step, i) => (
              <Reveal key={step.step} delay={i * 80} className="flex flex-col gap-2">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {TRUST_POINTS.map((point, i) => (
              <Reveal key={point.title} delay={i * 80}>
                <Card className="border-border shadow-none hover:shadow-md transition-shadow">
      ...
      <section className="py-16 bg-secondary/50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <Reveal>
              <FlameBadge className="mb-4">Samagri made easy</FlameBadge>
            ...
            <Reveal delay={150} className="grid grid-cols-2 gap-4">
              {(["GRIHA_PRAVESH", "SATYANARAYAN_PUJA", "UPANAYANAM", "SEEMANTHAM"] as CeremonyType[]).map((type) => {
                const c = CEREMONIES[type]
                return (
                  <div key={type} className="rounded-lg border border-border bg-card p-4 hover:shadow-md transition-shadow">
      ...
      {/* CTA for priests — the lamp closes the ceremony too */}
      <section className="dusk-section relative overflow-hidden">
        <Reveal className="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-20 text-center flex flex-col items-center">
          <Flame glow className="h-16 w-auto mx-auto mb-6" />
          <FlameBadge tone="dark" className="mb-4">Founding Priest</FlameBadge>
          <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight mb-3 text-[#f2ead9]">
            Are you a priest?
          </h2>
          <p className="text-[#f2ead9]/70 mb-6 max-w-xl mx-auto leading-relaxed">
