...
export function PriestCard({ priest }: PriestCardProps) {
  ...
  return (
    <Card className="border-border/60 shadow-none hover:shadow-md hover:border-primary/30 transition-all group">
        ...
        <div className="flex gap-4">
          ...
          <div className="relative shrink-0">
            <Avatar className="h-16 w-16 rounded-xl">
              ...
              <AvatarFallback className="rounded-xl bg-accent text-primary font-semibold text-lg">
                {initials}
              </AvatarFallback>
            </Avatar>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-2">
              ...
              <span className="text-xs font-medium">{priest.rating.toFixed(1)}</span>
              <span className="text-xs text-muted-foreground">({priest.reviewCount} reviews)</span>
              <span className="text-muted-foreground">·</span>
              <span className="text-xs text-muted-foreground">{priest.experienceYears}y exp</span>
            </div>

            {/* Languages */}
            <div className="flex flex-wrap gap-1 mt-2">
              {priest.languages.slice(0, 3).map((lang) => (
                <Badge key={lang} variant="outline" className="text-xs px-1.5 py-0 h-5 border-border/60">
                  {LANGUAGES[lang]}
                </Badge>
              ))}
            </div>
